// This app's implementation of @wealthai/core's PlatformAdapter.
//
...
// `response.body` is null and the turn dies before its first token.
// `expo/fetch` is WinterCG-compliant and supports body streaming, which is
...
const astroAdapter: PlatformAdapter = {
  ...
  // Multipart uploads go through RN's built-in fetch: its FormData accepts
  // {uri,name,type} descriptors and streams files natively — expo/fetch
  // (WinterCG-strict) rejects those parts outright.
...
export function ensureCoreInitialized(): void {
  if (!isCoreInitialized()) initCore(astroAdapter);
}
