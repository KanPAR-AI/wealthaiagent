// lib/analytics.ts — Firebase Analytics (GA4) for the astrology app.
//
// This app has its OWN Firebase registrations (docs/49 ASTRAL-68 D10), so its
// events land in their own stream rather than mixing with apps/mobile's —
// which is the point: §12's funnel is per-product. Same wrapper as mobile's
// so the two read alike; a divergence here would be a divergence in what the
// funnel means.
//
