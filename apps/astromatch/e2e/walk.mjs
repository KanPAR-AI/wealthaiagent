...
try {
  ...
  // PH-41 flipped the last three capabilities, so the assertion flips with
  // them: there is no `false` left in the map, which means every control is
  // PRESENT — and the check that matters now is that none of them is a dead
  // affordance. (The PH-39 version of this check was written as a double
  // negative and would have passed either way; it was replaced, not relaxed.)
  for (const [id, what] of [
    ['snapshot', 'the camera'],
    ['selection', 'the selection read'],
    ['shortlist-open', 'the shortlist'],
    ['manual', 'manual entry'],
    ['paste', 'paste'],
  ]) {
    check((await panel.locator(`[data-testid="${id}"]`).count()) === 1, `${what} IS rendered`);
  }
  check(
    !controls.some((c) => /coming soon|not yet|stay tuned/i.test(c ?? '')),
    'and nothing on the screen promises a thing this build cannot do',
  );
  const disabled = await panel.$$eval('button', (els) =>
    els.filter((b) => b.disabled).map((b) => b.textContent?.trim()),
  );
  check(
    disabled.length === 0,
    `no control is greyed out on the way in (${JSON.stringify(disabled)})`,
  );
  ...
  const chipAnswered = await cam
    .waitForFunction(
      ...
      // object passed SECOND becomes the ARGUMENT, and the wait keeps the
      // default 30 s — so a model chip that answered in 40 s was reported as
      // one that never came back. `undefined` is the argument.
      undefined,
      { timeout: 180_000 },
    )
    .then(() => true)
  ...
  // The legs live in `e2e/legs-ph41.mjs` so `e2e/ph41-walk.mjs` can run THE
  // SAME code without the twenty-four legs before them — which matters when
  // the local backend is reloading under another agent's edits (F307) and a
  // streamed turn early in this file dies before PH-41 is reached.
