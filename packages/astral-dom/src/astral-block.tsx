/**
 * The React-DOM host for the astrology blocks (docs/49 ASTRAL-15/16/17/20;
 * moved into this package by docs/73 ASTRAL-321).
 *
 * TWO DOM surfaces render these now — the web chat and the AstroMatch
 * extension's 380 px side panel — so the dispatcher lives in a package and
 * the one app-local thing it used to reach for (the web's `chat-quick-reply`
 * CustomEvent) is injected through `host.ts`. See that file for why a copy
 * into the extension would have compiled rather than failed.
 *
    ...
    if (request) {
      body = (
        <InputRequestView
          ...
          width={width}
          request={request}
          // The answer leaves through the HOST. On the web that is the same
          // channel the shipped quick-reply widgets use; in the extension
          // panel it is the service-worker transport. Either way what
          // travels is a typed fence, not a sentence for the extractor to
          // re-parse (docs/49 ASTRAL-85, F18) — this file builds no message
          // and names no channel.
          onSend={(text) => getAstralDomHost().send(text)}
        />
      );
    }
